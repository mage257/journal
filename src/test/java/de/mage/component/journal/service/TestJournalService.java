...
import de.mage.component.journal.domain.AddItemRequest.Allocation;
...
public class TestJournalService extends IntegrationTestsSupport {
  ...
  public void givenExistingJournal_whenAddingItem_shouldSucceed() {
    ...
    request.setIdentifier(this.randomString(32));
    request.setValueDate(LocalDate.now().format(DateTimeFormatter.ISO_DATE));
    request.setCurrencyCode("EUR");

    final Long sequence = this.journalRequestProcessor.process(request);
    ...
    final TransitionJournalRequest scheduleRequest = new TransitionJournalRequest();
    scheduleRequest.setAction(Action.SCHEDULE);
    this.journalRequestProcessor.process(sequence, scheduleRequest);

    final TransitionJournalRequest releaseRequest = new TransitionJournalRequest();
    releaseRequest.setAction(Action.RELEASE);
    this.journalRequestProcessor.process(sequence, releaseRequest);
